...
class Kriteria extends CI_Controller
{
    ...
    function index()
    {
        ...
        $this->load->view(akses().'/data/kriteria/kriteriaview',$d);
    ...
    function add()
    {
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			if($this->mod_kriteria->kriteria_add($nama)==TRUE)
			{
				...
				redirect(base_url(akses()).'/data/kriteria');
			}else{
				set_header_message('danger','Tambah Kriteria','Gagal menambah kriteria');
				redirect(base_url(akses()).'/data/kriteria/add');
			}
		}else{
			$meta['judul']="Tambah Kriteria";
        	$this->load->view('tema/header',$meta);
        	$this->load->view(akses().'/data/kriteria/kriteriaadd');
        	$this->load->view('tema/footer');
		}
	}
	...
	function edit()
    {
		...
		if($this->form_validation->run()==TRUE)
		{
        	...
        	$this->load->view(akses().'/data/kriteria/kriteriaedit',$d);
        	$this->load->view('tema/footer');
		}
	}
	...
	function subkriteria()
	{
        ...
        $this->load->view(akses().'/data/kriteria/subkriteria/subkriteriaview',$d);
        $this->load->view('tema/footer');
	}
	...
	function subkriteriaadd()
	{
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			if($this->mod_kriteria->subkriteria_add($tipe,$kriteriaid,$opmax,$isi,$opmin,$min,$nilaiid)==TRUE)
			{
				...
				redirect(base_url(akses().'/data/kriteria/subkriteria').$link);
			}else{
				set_header_message('danger','Tambah Parameter','Gagal menambah parameter');
				redirect(base_url(akses().'/data/kriteria/subkriteriaadd').$link);
			}
		}else{
			$kriteria=$this->input->get('kriteria');
	        ...
	        $d['kriteria']=$kriteria;
	        $d['link']=$link;
	        $this->load->view(akses().'/data/kriteria/subkriteria/subkriteriaadd',$d);
	        $this->load->view('tema/footer');
		}
	}
    ...
    {		
		...
		{			
	        ...
	        $this->load->view(akses().'/data/kriteria/subkriteria/subkriteriaedit',$d);
	        $this->load->view('tema/footer');
		}
	}
