...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/master/Alternatif/updateutama",
		data:$(this).serialize(),
		error:function(){
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_alternatif($kriteria_id,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
...
<div class="pull-right">
	<button type="submit" class="btn btn-success">Simpan Alternatif</button>
</div>
<?php
echo form_close();
?>
...
<table class="table table-bordered">
<thead>
	<th colspan="<?=$jumlah+1;?>" class="text-center">Matrik Nilai Alternatif</th>
</thead>
<thead>
	<th>Alternatif</th>
	<?php
